...
func getMysqlData(fans int, uid int, count int, page int, db *sql.DB) []*EventLog {
	...
	var sql string
	if fans >= c.fansLimit {
		sql = "select type,uid,created,infoid,status,tid from `event_log` where status=1 and uid=" + strconv.Itoa(uid) + " order by id desc limit " + strconv.Itoa(c.numloops) + " offset " + strconv.Itoa(offset)
		...
		sql = "select type,uid,created,infoid,status,tid from `event_log` where status=1 and uid=" + strconv.Itoa(uid) + " and created >='" + c.dateLimit + "' order by id desc limit " + strconv.Itoa(c.numloops) + " offset " + strconv.Itoa(offset)
