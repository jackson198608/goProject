...
func testRpush(client *redis.Client) {
	...
	iosStr := iosRedisString + "^" + mongoString
	androidStr := androidRedisString + "^" + mongoString

	for i := 0; i < numForOneLoop; i++ {
		if i%2 == 0 {
			err := (*client).RPush("MessageCenter", iosStr).Err()
			if err != nil {
				//insert error
			}
		} else {
			err := (*client).RPush("MessageCenter", androidStr).Err()
			if err != nil {
				//insert error
			}
		}
