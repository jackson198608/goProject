...
//@todo for sogou
func qSogouHome(p *page.Page) {
	query := p.GetHtmlParser()

	//find weixin account homepage
	query.Find(".gzh-box2 .txt-box .tit a").EachWithBreak(func(i int, s *goquery.Selection) bool {
		url, isExsit := s.Attr("href")
		if isExsit {
			realUrlTag := "weixinAccountHome"
			req := newRequest(realUrlTag, url)
			p.AddTargetRequestWithParams(req)
		}
		return false
...
//@todo for sogou
func qWeixinAccountHome(p *page.Page) {
	query := p.GetHtmlParser()

	//find weixin account homepage
	query.Find(".page_profile_info .txt-box .tit a").EachWithBreak(func(i int, s *goquery.Selection) bool {
		url, isExsit := s.Attr("href")
		if isExsit {
			realUrlTag := "weixinAccountHome"
			req := newRequest(realUrlTag, url)
