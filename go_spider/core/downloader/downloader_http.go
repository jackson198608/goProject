...
func (this *HttpDownloader) downloadFile(p *page.Page, req *request.Request) (*page.Page, string) {
	...
	var bodyStr string
	if p.GetUrlTag() == "shopImage" {

		b, _ := ioutil.ReadAll(resp.Body)
		bodyStr = string(b)
		//mlog.StraceInst().Println("getContentLength : " + strconv.Itoa(len(bodyStr)))
...
func (this *HttpDownloader) downloadHtml(p *page.Page, req *request.Request) *page.Page {
	...
	//mlog.StraceInst().Println("getContentLength : " + strconv.Itoa(len(destbody)))
	...
	if p.GetUrlTag() == "shopImage" {
		...
		tempBody := p.GetBodyStr()
		mlog.StraceInst().Println("Length : " + strconv.Itoa(len(tempBody)))
		return p
	}
	var body string
...
func (this *HttpDownloader) downloadText(p *page.Page, req *request.Request) *page.Page {
	p, destbody := this.downloadFile(p, req)
	if !p.IsSucc() {
		mlog.StraceInst().Println(destbody)
		return p
	}
